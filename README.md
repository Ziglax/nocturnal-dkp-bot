...
- **Short auction** (`/startbid`) — lives in memory for `bidtime` seconds. The bot posts the item in the auction channel with *Main bid* / *Alt bid* / *Cancel* buttons and rings a bell in the raid channel(s). Clicking a bid button opens a Discord form asking for the amount; entering **0** withdraws the bid. At the end the winners are announced and the officer who started it presses **Confirm Winner/s** (within 6 minutes) to debit the DKP. A winner whose balance no longer covers their bid by then loses the item to the next bid down, and the message says which bid was skipped. An officer can end it early with **Cancel**: no winner is picked and no DKP is taken, and the post turns red, says *Bid cancelled* and records when it was cancelled and by whom.
- **Long auction** (`/startlongbid`) — stored in the database, runs for N hours. The bot posts the item in the long auction channel with the same *Main bid* / *Alt bid* buttons and form as a short auction; players bid until the end time; the bot closes it `lockdelay` minutes later (20 by default) and edits the original auction post with the winners, their bids and whether their DKP was taken (no new message, nobody is pinged). At close every bid is re-checked against the bidder's balance at that moment: a bid the player can no longer cover (e.g. after winning a short auction meanwhile) is dropped silently. Winners are debited automatically when the auction closes, unless it was started with `autodebit: false` — then the closed post carries a **Confirm DKP** button for officers instead. Whenever the DKP are taken, a winner whose balance no longer covers their bid loses the item to the next bid down, and the **DKP** field names the bid that was skipped.

  An officer can pull a long auction with `/cancelauction <auctionid>`: bids stop at once, no winner is picked, no DKP is taken, and the post turns red and records who cancelled it and when. The bids are kept in the database and `/auctiondetails` reads them back, but they are **not** republished in the channel — a cancelled auction is usually re-run, and reprinting everyone's first bid would hand the second round to whoever scrolls up. `/endauction <auctionid>` does the opposite: it closes a running auction immediately, ignoring both the remaining time and the `lockdelay`, and settles it exactly as the bot would have — same winners, same hand-down to the next bid, same automatic debit, same recap — with the results published straight away. The auction stops taking bids the instant the command runs, before the winners are worked out, and its end time is rewritten to that moment: the recap then names the time the auction actually stopped rather than the date it was originally scheduled to end.
...
**Permissions.** Officer commands require either the **Administrator** permission or the role set with `/configure role` (the *officer role*). `/cancelauction` and `/endauction` are the exception: once an officer role is configured they require **that role**, and an administrator who does not have it is refused — pulling or force-closing a live auction moves DKP, and the guild already said who may do that. With no officer role configured they fall back to **Administrator** like everything else, so a server that has not run `/configure` yet is not left unable to stop a 48-hour auction. `/configure`, `/showconfig` and `/backup` are hidden from non-administrators by default on the Discord side (changeable in *Server Settings → Integrations*); `/configure` is also an officer command, but `/showconfig` and `/backup` have no bot-side check, so anyone granted them in Integrations can run them. All commands must be used inside a server, not in DMs.
...
| `/cancelauction <auctionid>` | Officer role | Voids a **running** long auction: bids stop immediately, no winner is picked and no DKP is taken. The post turns red, loses its bid buttons and records who cancelled it and when. The bids stay in the database and are readable with `/auctiondetails`, but are not republished. Refused on an auction that is already closed — that one may already have moved DKP, and giving it back is `/adddkp`'s job. Unlike the other officer commands, the **Administrator** permission on its own is not accepted here when an officer role is configured. |
| `/endauction <auctionid>` | Officer role | Closes a **running** long auction now instead of at its end time, and publishes the results immediately — the `lockdelay` is skipped. Bidding stops the moment the command runs, and the auction's end time is rewritten to that moment. Winners, the hand-down to the next bid when a winner cannot pay, the automatic debit and the recap are exactly what the bot would have produced on its own: it is the same code. The reply names the winners, and warns instead if the auction message could not be updated — the auction is closed either way and whatever DKP it was going to take has already been taken, but nobody was told. Watch that case on an auction started with `autodebit: false`: the **Confirm DKP** button is drawn on the post that could not be updated, so it is lost with it and the winners have to be charged with `/removedkp`. Same officer-role restriction as `/cancelauction`. Skipping the `lockdelay` means this auction's prices become public while any auctions running beside it are still taking bids. |
| `/auctiondetails <auctionid>` | Officer | Shows the bids and winners of a **finished** auction (ephemeral, split over several messages if long). A running auction is refused: officers bid too, so the standing bids of a live auction are shown to nobody. A long auction only becomes readable once the bot closes it, `lockdelay` minutes after its end time (20 by default). Short auctions are readable as soon as they end (their Auction ID is printed on the winners embed; cancelled ones are not stored). A long auction cancelled with `/cancelauction` is readable straight away, and the details say so and name the officer who cancelled it. Once the details have been shown, the auction channel gets a public notice that you peeked — nothing is posted when the command refuses. |
...
An auction can end while the form is still open — a short one lasts a couple of minutes, and Discord keeps a form open for fifteen. Submitting it then answers that the auction is over and that nothing was changed, naming the item. The same answer covers a form submitted after a bot restart, and one abandoned for more than fifteen minutes. An auction an officer cancelled answers that it was cancelled, not that it ended.

The buttons of a long auction keep working across a bot restart — unlike a short auction, it is stored in the database and its buttons are answered from the auction id they carry rather than by a listener that would die with the process. They disappear when the bot closes the auction, replaced by a **Confirm DKP** button when a winner still owes DKP. That button is restricted to administrators and the officer role, survives a restart the same way, and is safe to press twice: each winner is claimed on the auction document before their DKP is written, so nobody is charged twice, and a second press landing while the first is still running is refused rather than run alongside it — and it disappears once every winner has paid.
...
- A winner whose balance no longer covers their bid when the DKP are taken loses the item to the next bid down, and the recap names the skipped bid. Only when no bid is left to hand it to does the winner keep the item undebited: the embed says so and the **Confirm DKP** button stays, to be pressed once they can cover it.
...
- `/cancelauction` voids a long auction but never deletes it: the document stays in `auctions` with `cancelled: true` and its bids intact, and `/auctiondetails` is the only place those bids are shown. A long auction that has already **closed** still cannot be undone — give the DKP back with `/adddkp`.
