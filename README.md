...
- **Long auction** (`/startlongbid`) — stored in the database, runs for N hours. The bot posts the item in the long auction channel with the same *Main bid* / *Alt bid* buttons and form as a short auction; players bid until the end time; the bot closes it `lockdelay` minutes later (20 by default) and edits the original auction post with the winners, their bids and whether their DKP was taken (no new message, nobody is pinged). At close every bid is re-checked against the bidder's balance at that moment: a bid the player can no longer cover (e.g. after winning a short auction meanwhile) is dropped silently. Winners are debited automatically when the auction closes, unless it was started with `autodebit: false` — then the closed post carries a **Confirm DKP** button for officers instead. Whenever the DKP are taken, a winner whose balance no longer covers their bid loses the item to the next bid down, and the **DKP** field names the bid that was skipped.
  ...
  An officer can pull a long auction with `/cancelauction <auctionid>`: bids stop at once, no winner is picked, no DKP is taken, and the post turns red and records who cancelled it and when. The bids are kept in the database and `/auctiondetails` reads them back, but they are **not** republished in the channel — a cancelled auction is usually re-run, and reprinting everyone's first bid would hand the second round to whoever scrolls up. `/endauction <auctionid>` does the opposite: it closes a running auction immediately, ignoring both the remaining time and the `lockdelay`, and settles it exactly as the bot would have — same winners, same hand-down to the next bid, same automatic debit, same recap — with the results published straight away.
...
| `/endauction <auctionid>` | Officer role | Closes a **running** long auction now instead of at its end time, and publishes the results immediately — the `lockdelay` is skipped. Winners, the hand-down to the next bid when a winner cannot pay, the automatic debit and the recap are exactly what the bot would have produced on its own: it is the same code. The reply names the winners. Same officer-role restriction as `/cancelauction`. Skipping the `lockdelay` means this auction's prices become public while any auctions running beside it are still taking bids. |
