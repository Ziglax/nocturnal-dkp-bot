...
Bidding on a short auction requires the player to have DMs open for the server: the amount is asked by DM (60 s to answer; `0` closes the prompt without bidding). A placed bid cannot be withdrawn, but bidding again replaces it. Bidders must already have a player record.
